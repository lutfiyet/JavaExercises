package CollectionsExercises;

import java.util.ArrayList;
import java.util.Arrays;

public class _10_arraylist12 {

    /*
  Create a method named rangeBtw().
     An Arraylist and two separate ints as parameters
     return type int

      Count how many values the Arraylist has between two ints.
     return count (return the counter (count))

     Example:
     ArrayList: 1 , 5 , 22, 10 ,20 ,14 , 8 , 25 , 30, 28
     min = 20
     max = 30
     return = 5 (20,22,25,28,30)
     Including the min and max range (includes 20 and 30 in Example1).
     */

    public static void main(String[] args) {

        ArrayList<Integer>arrayList = new ArrayList<>(Arrays.asList(1 , 5 , 22, 10 ,20 ,14 , 8 , 25 , 30, 28));

        int min = 20;
        int max = 30;

        System.out.println(rangeBtw(arrayList,min,max));


    }
    public static int rangeBtw(ArrayList<Integer>arrayList, int min, int max){

        int count =0;

        for (int i = 0; i < arrayList.size() ; i++) {

            if (arrayList.get(i) >= min && arrayList.get(i) <= max){
                count++;
            }
        }
        return count;
    }
}